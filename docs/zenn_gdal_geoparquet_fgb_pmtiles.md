...
- 自治体や企業で、GIS データを GeoParquet や FGB・PMTiles で公開・配信したい担当者
...
#### 試してみたいテーマ：PostgreSQL（PostGIS）からの変換

既存の GIS では、**PostgreSQL + PostGIS** にデータを格納しているケースが多くあります。SHP だけでなく、**DB から直接 GeoParquet / FGB / PMTiles へ変換** できると、バックアップや公開用スナップショットの作成がしやすくなります。GDAL には PostgreSQL 用の OGR ドライバがあり、`ogr2ogr` で `PG:dbname=...` のような接続文字列を指定して読み出せます。一方で、次のような課題が考えられます。

- **接続・認証**: 本番 DB への接続権限、ファイアウォール、SSL の要否。読み取り専用ユーザやスナップショット用のレプリカを用意する必要がある場合がある。
- **スキーマ・複数テーブル**: 1 データベースに複数スキーマ・複数テーブル（レイヤ）がある場合、**どのテーブルを対象にするか** の選定と、テーブルごとのループやジョブ設計。ビューやマテリアライズドビューを経由するかどうか。
- **ジオメトリ型・CRS**: PostGIS は多様なジオメトリ型（Geometry/Geography、SRID 混在）を許容する。GeoParquet や FGB に出すときに、**型の正規化**（例: すべて MultiPolygon にそろえる）や **CRS の統一** が必要になることがある。SHP で起きたような Polygon/MultiPolygon 混在による FGB の 0 件問題と同様の考慮がいる可能性。
- **データ量・タイムアウト**: テーブルが大きい場合、フルダンプでメモリや時間がかかる。**WHERE 条件で範囲や年度を絞る**、**バッチ分割**（例: タイルや ID 範囲ごと）してから変換するなどの設計が必要になりそう。
- **依存関係**: GDAL の PostgreSQL ドライバは libpq に依存。WSL 上では `libpq-dev` を入れておけばビルド時に検出されることが多いが、**ビルドオプション** で PostgreSQL サポートが有効になっているか確認が必要。

これらを整理したうえで、同じ「隔離ビルド＋GDAL のみ」のスタイルで、PostgreSQL → GeoParquet → FGB/PMTiles のパイプラインを試してみる予定です。
