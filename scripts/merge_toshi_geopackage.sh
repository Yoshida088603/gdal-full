#!/usr/bin/env bash
# データ_geopackage_converted/都市部官民基準点等データ 内の全 .gpkg を 1 本にマージする。
# 各 GPKG は csv_to_geopackage.sh で既に EPSG:3857 になっているため SRS 変換は不要。
# 使い方: source env.sh && bash scripts/merge_toshi_geopackage.sh
# 出力: データ_geopackage_marged/都市部官民基準点等_merged.gpkg（レイヤ名: toshi_merged）

set -e
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
DATA_PARENT="$PROJECT_ROOT/inputfile/20260219昨年納品DVD/05ホームページ公開用データ及びプログラム"
INPUT_DIR="${DATA_PARENT}/データ_geopackage_converted/都市部官民基準点等データ"
OUTPUT_FILE="${DATA_PARENT}/データ_geopackage_marged/都市部官民基準点等_merged.gpkg"
...
echo "Merge 都市部官民基準点等: ${#gpkgs[@]} files -> $OUTPUT_FILE" >&2
first=true
merged=0
for gpkg in "${gpkgs[@]}"; do
  base=$(basename "$gpkg" .gpkg)
  if [[ "$first" == true ]]; then
    ogr2ogr -f GPKG -nln toshi_merged \
      "$OUTPUT_FILE" "$gpkg" || { echo "Warning: failed $base" >&2; continue; }
    first=false
    ((merged++)) || true
  else
    ogr2ogr -update -append -nln toshi_merged \
      "$OUTPUT_FILE" "$gpkg" || { echo "Warning: failed $base" >&2; continue; }
    ((merged++)) || true
  fi
